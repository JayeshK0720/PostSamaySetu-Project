...
def reset_password():
    if request.method == 'POST':
        ...
        # Get user id from session or relevant logic
    ...
    if request.method == 'POST':
        ...
        if 'profile_pic' in request.files:
            ...
            if profile_pic and profile_pic.filename != '':
                ...
                with sqlite3.connect('delivery.db') as conn:
                    c = conn.cursor()
                    c.execute('UPDATE users SET username=?, email=?, phone=?, address=?, location=?, profile_pic=? WHERE id=?',
                              (username, email, phone, address, location, pic_filename, session['user_id']))
                    conn.commit()
            else:
